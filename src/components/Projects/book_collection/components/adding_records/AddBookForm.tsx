...
const AddBookForm = forwardRef<AddBookFormRef, AddBookFormProps>((props, ref) => {
  ...
  const [inBookSeries, setInBookSeries] = useState(editableData?.bookSeries ? true : false);
  ...
  const [titleEnglish, setTitleEnglish] = useState(editableData?.titleEnglish || '');
  const [titleOriginal, setTitleOriginal] = useState(editableData?.titleOriginal || '');
  const [language, setLanguage] = useState(uploadedLanguage || editableData?.language || 'Polish');
  const [genres, setGenres] = useState<string[]>([]);
  const [publisher, setPublisher] = useState(uploadedPublisher || editableData?.publisher || '');
    ...
    return (
      <form className='bookCollection__addBook__bookForm__form addBookForm'>
        ...
        <div className='addBookForm_element addBookForm_element_publisher'>
          ...
          >
            <option value={publisher.id}>{publisher.name || '-- find me --'}</option>
            {data.publishers.map((publisher: { id: string; name: string }) => {
              return <option key={publisher.id} value={publisher.id} label={publisher.name}></option>;
            })}
          </select>
        </div>
        {/* Genre Selection */}
        ...
        <div className='addBookForm_element addBookForm_element_isBookSeries'>
          ...
          <label htmlFor='yes' className='form-control-radio'>
            <input type='radio' name='in_bookSeries' id='yes' onClick={() => setInBookSeries(true)} />
            Yes
          </label>
          <label htmlFor='no' className='form-control-radio'>
            <input type='radio' name='in_bookSeries' id='no' onClick={() => setInBookSeries(false)} />
            No
          </label>
        </div>
        {inBookSeries && (
